...
**Status:** FAIL
**Timestamp:** 2026-06-30T04:28:23
**Run ID:** `2026-06-30_04-28_Frontend-Validation-Bundle---Invalid`
...
| IssueSeverity.information | Bundle.entry[1].resource/*Observation/txn-obs-1*/.value.ofType(Quantity) | Reference to draft CodeSystem http://unitsofmeasure.org\|2.1.1 |
| IssueSeverity.information | Bundle.entry[1].resource/*Observation/txn-obs-1*/ | Validate Observation against the Heart rate profile (http://hl7.org/fhir/StructureDefinition/heartrate) which is required by the FHIR specification because the LOINC code 8867-4 was found |
| IssueSeverity.information | Bundle.entry[1].resource | Validate Observation against the Heart rate profile (http://hl7.org/fhir/StructureDefinition/heartrate) which is required by the FHIR specification because the LOINC code 8867-4 was found |
...
| IssueSeverity.warning | Bundle.entry[1].resource | Best Practice Recommendation: In general, all observations should have an effective[x] () |
